...
export const getStoragePath = ({svc, userId, postId}: {svc: STORAGE_SVC; userId: string; postId?: string}) => {
  return svc === STORAGE_SVC.user ? getUserPath(userId) : getPostStoragePath(userId, postId!)
}

export function getParentRef(p: {storage: FirebaseStorage; svc: STORAGE_SVC; userId: string; parentId?: string}) {
  if (p.svc === 'user') {
...
function completeRef(filename: string, parent: StorageReference): StorageReference {
  return ref(parent, filename)
}

export async function uploadFile(parent: StorageReference, fs: FileList | File[]): Promise<string[]> {
  const refers: Array<StorageReference> = []
  for (let i = 0; i < fs.length; i++) {
    refers.push(completeRef(uuidv4(), parent))
  }
  if (refers.length !== fs.length) throw Error('반드시 참조와 파일 목록은 길이가 같아야합니다.')
  const urls: string[] = []
