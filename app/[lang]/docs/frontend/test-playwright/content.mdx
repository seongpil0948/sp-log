...
## 시작하기전에
만약 당신이 [프론트엔드 테스팅 기본](./test-theory)를 읽지 않았다면 읽고 오시길 바랍니다.
### 예시 레포지터리
...
![install_extension2.png](/docs/playwright/playwright/install_extension2.png)
	2. install 버튼을 클릭하여 설치합니다.
![playwright_extension.png](/docs/playwright/playwright/playwright_extension.png)
...
6. `pnpm run test:e2e:ui` 를 실행하면 아래 사진과 같이 윈도우를 확인할 수 있습니다.
    ![ui_mode_blank.png](/docs/playwright/playwright/ui_mode_blank.png)
7. 위사진의 좌측 사이드바의 테스트 케이스 목록을 클릭하여 재생(Run) 버튼을 눌러주세요.
    아래와 같이 녹색 체크 표시가 보인다면, **축하합니다** 당신은 테스트에 성공했습니다!
    ![playwright-ui-allapssed.png](/docs/playwright/playwright-ui-allapssed.png)
...
![filter_list.png](/docs/playwright/playwright/filter_list.png)
...
6. 커서를 `await page.goto(MAIN_ROUTE)` 하단 9번 라인에 위치 시킵니다.
    ![record_at_cursor.png](/docs/playwright/record_at_cursor.png)
...
12. 테스트 UI를 통해 테스트를 실행합니다.   
		...
		2. ![run_create_todo_test.png](/docs/playwright/run_create_todo_test.png)
13. 저런 테스트가 실패했습니다! 살펴봅시다.  **[사진 12]**  
![ui-test-fail.png](/docs/playwright/ui-test-fail.png)
...
14. 두 번째 행을 추가하는 테스트 코드 작성
    ...
    3.  ![add_todo.png](/docs/playwright/add_todo.png)
...
| ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| [page.getByRole()](https://playwright.dev/docs/locators#locate-by-role)               | 요소의 역할에 따라 선택                                                                             | `Sign in`                                                            | `await page.getByRole('button', { name: 'Sign in' }).click();`                      |
| [page.getByLabel()](https://playwright.dev/docs/locators#locate-by-label)             | 라벨 Text 에 따라 선택                                                                              | `Password`                                                           | `await page.getByLabel('Password').fill('secret');`                                 |
| [page.getByPlaceholder()](https://playwright.dev/docs/locators#locate-by-placeholder) | placeholder Text 에 따라 선택                                                                       |                                                                      | `await page.getByPlaceholder("name@example.com").fill("playwright@microsoft.com");` |
| [page.getByText()](https://playwright.dev/docs/locators#locate-by-text)               | text에 따라 선택(substring, Regex 사용가능)                                                         | `Welcome, John`                                                      | `await expect(page.getByText(/welcome, [A-Za-z]+$/i)).toBeVisible();`               |
| [page.getByAltText()](https://playwright.dev/docs/locators#locate-by-alt-text)        | 대부분의 이미지 요소에 사용되는 alt 속성 기준 선택                                                  | `![playwright logo](https://playwright.dev/img/playwright-logo.svg)` | `await page.getByAltText('playwright logo').click();`                               |
| [page.getByTitle()](https://playwright.dev/docs/locators#locate-by-title)             | title 속성 기준 선택                                                                                | `25 issues`                                                          | `await expect(page.getByTitle('Issues count')).toHaveText('25 issues');`            |
| [page.getByTestId()](https://playwright.dev/docs/locators#locate-by-test-id)          | data-testid 속성 기준 선택, 테스트 용도로 구별되는 속성으로 일관성을 유지하기 위해 주로 사용합니다. | `방향`                                                               | `await page.getByTestId('directions').click();`                                     |
