...
*  
## kubelet kubeadm kubectl 세팅에 사용된 스크립트(CentOS)
...
init 과정에서 에러가 발생했다.  
```bash
kubeadm init   --pod-network-cidr=192.168.0.0/16   --cri-socket=unix:///var/run/crio/crio.sock --apiserver-advertise-address=192.168.0.103
```
에러 내용들을 살펴보니 API 서버에 연결할 수 없다는 내용이었다.
```
dial tcp 192.168.0.103:6443: connect: connection refused
E0412 10:21:12.276483    5638 memcache.go:265] couldn't get current server API group list: Get "http://localhost:8080/api?timeout=32s": dial tcp [::1]:8080: connect: connection refused
The connection to the server localhost:8080 was refused - did you specify the right host or port?
```
확인 명령어
```bash
$ sudo netstat -tulnp | grep 6443
$ ps -auxef | grep kube-apiserver
```
- f: full format
- e: environment
- x: all processes
...
### 해결방법
CRI-O 의 default  드라이버는 systemd 이다.  
찾아보니 [관련링크](https://tech.kakao.com/2020/06/29/cgroup-driver/) 권한 문제가 발생할 수 있다고 한다.  
cgroupfs cgroup 드라이버로 전환하려면, /etc/crio/crio.conf 를 수정하거나 /etc/crio/crio.conf.d 에 드롭-인(drop-in) 구성을 배치한다.
```
[crio.runtime]
conmon_cgroup = "pod"
