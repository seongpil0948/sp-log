...
export function ImageCard({src}: {src: string}) {
  ...
  return (
    <>
      <Image isZoomed onClick={onOpen} ref={imageRef} src={src} alt={src} width={300} loading="lazy" />
      <Modal size="full" isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {onClose => (
            <>
              <ModalHeader className="flex flex-col gap-1">{fileNameFromPath(src)}</ModalHeader>
              <ModalBody>
                <div onClick={onClose} className="flex h-full flex-col items-center justify-center align-middle">
